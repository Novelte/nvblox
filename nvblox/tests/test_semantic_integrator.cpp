...
SemanticImage generateSingleSemantImage(const Semantic& semantic, const int height,
                                   const int width) {
  // Generate a random color for this scene
  SemanticImage image(height, width);
  nvblox::test_utils::setImageConstantOnGpu(semantic, &image);
  return image;
}
...
float checkSphereSemantic(const SemanticLayer& semantic_layer, const Vector3f& center,
                       const float radius, const Semantic& semantic) {
  // Check that each sphere is colored appropriately (if observed)
  int num_observed = 0;
  int num_tested = 0;
  auto check_semantic = [&num_tested, &num_observed](
                         const SemanticVoxel& voxel,
                         const Semantic& semantic_2) -> void {
    ++num_tested;
    printf("voxel.weight: %f\n", voxel.weight);
    if (voxel.weight >= 1.0f) {
      EXPECT_EQ(voxel.id, semantic_2);
      ++num_observed;
    }
  };
  ...
  for (const Vector3f p : sphere_points) {
    ...
    EXPECT_TRUE(getVoxelAtPosition<SemanticVoxel>(semantic_layer, p, &semantic_voxel));
    check_semantic(*semantic_voxel, semantic);
  }
...
TEST_F(SemanticIntegrationTest, IntegrateSemantToGroundTruthDistanceField) {
  ...
  // Semantic layer
  SemanticLayer semantic_layer(voxel_size_m_, MemoryType::kDevice);

  // Generate a single semant for this scene
  const Semantic semantic = Semantic::Unknown();
  const SemanticImage image = generateSingleSemantImage(semantic, height_, width_);
  ...
  for (size_t i = 0; i < kNumTrajectoryPoints; i++) {
    ...
    // Generate an image with a single semant
    std::vector<Index3D> updated_blocks;
    semantic_integrator.integrateFrame(image, T_S_C, camera_, gt_layer_,
                                    &semantic_layer, &updated_blocks);
  ...
  SemanticLayer semantic_layer_host(semantic_layer, MemoryType::kHost);

  // Lambda that checks if voxels have the passed semant (if they have weight >
  // 0)
  auto semantic_check_lambda = [&semantic](const Index3D& voxel_idx,
                                     const SemanticVoxel* voxel) -> void {
    if (voxel->weight > 0.0f) {
      EXPECT_EQ(voxel->id, semantic);
    }
  };
  ...
  for (const Index3D& block_idx : touched_blocks) {
    callFunctionOnAllVoxels<SemanticVoxel>(
        *semantic_layer_host.getBlockAtIndex(block_idx), semantic_check_lambda);
  }
  ...
  for (const Vector3f p : sphere_points) {
    const SemanticVoxel* semantic_voxel;
    EXPECT_TRUE(
        getVoxelAtPosition<SemanticVoxel>(semantic_layer_host, p, &semantic_voxel));
    if (semantic_voxel->weight >= 1.0f) {
  ...
  for (const Index3D block_idx : semantic_layer_host.getAllBlockIndices()) {
    EXPECT_NE(gt_layer_.getBlockAtIndex(block_idx), nullptr);
  }
}

TEST_F(SemanticIntegrationTest, SemanticSpheres) {
  ...
  const auto semantic_1 = Semantic(0);
  const auto semantic_2 = Semantic(1);
  const auto semantic_3 = Semantic(2);

  const auto image_1 = generateSingleSemantImage(semantic_1, height_, width_);
  const auto image_2 = generateSingleSemantImage(semantic_2, height_, width_);
  const auto image_3 = generateSingleSemantImage(semantic_3, height_, width_);

  std::vector<Index3D> updated_blocks;
  semantic_integrator.integrateFrame(image_1, T_S_C1, camera, gt_layer_,
                                  &semantic_layer, &updated_blocks);
  semantic_integrator.integrateFrame(image_2, T_S_C2, camera, gt_layer_,
                                  &semantic_layer, &updated_blocks);
  semantic_integrator.integrateFrame(image_3, T_S_C3, camera, gt_layer_,
                                  &semantic_layer, &updated_blocks);
  ...
  // mesh_integrator.colorMesh(semantic_layer, &mesh_layer);

  // ColorLayer color_layer_host(semantic_layer, MemoryType::kHost);
...
TEST_F(SemanticIntegrationTest, OcclusionTesting) {
  ...
  // Integrate a semantic image
  ProjectiveSemanticIntegrator semantic_integrator;
  SemanticLayer semantic_layer(voxel_size_m_, MemoryType::kDevice);

  const auto semant_1 = Semantic::Unknown();
  const auto image_1 = generateSingleSemantImage(semant_1, height_, width_);

  std::vector<Index3D> updated_blocks;
  semantic_integrator.integrateFrame(image_1, T_S_C, camera_, gt_layer_,
                                  &semantic_layer, &updated_blocks);
  semantic_integrator.integrateFrame(image_1, T_S_C, camera_, gt_layer_,
                                  &semantic_layer, &updated_blocks);
  SemanticLayer semantic_layer_host(semantic_layer, MemoryType::kHost);
  ...
  const float sphere_1_observed_ratio =
      checkSphereSemantic(semantic_layer_host, center_1, kSphereRadius, semant_1);
  EXPECT_GT(sphere_1_observed_ratio, 0.2);
  std::cout << "sphere_1_observed_ratio: " << sphere_1_observed_ratio
            << std::endl;
  ...
  for (const Vector3f p : sphere_points) {
    ...
    const bool block_allocated =
        getVoxelAtPosition<SemanticVoxel>(semantic_layer_host, p, &semantic_voxel);
    if (block_allocated) {
      EXPECT_EQ(semantic_voxel->weight, 0.0f);
    }
  }
}
