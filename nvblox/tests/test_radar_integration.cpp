...
class RadarIntegrationTest : public ::testing::Test {
 protected:
  RadarIntegrationTest()
      : radar(num_azimuth_divisions, num_elevation_divisions,
  ...
  const float horizontal_fov_rad = 60.0f * M_PI / 180.0;
...
Eigen::MatrixX3f generateSpherePointcloud(int num_azimuth_divisions,
  ...
  const float azimuth_increments_rad = 
      horizontal_fov_rad / (num_azimuth_divisions - 1);
  ...
  for (int az_idx = 0; az_idx < num_azimuth_divisions; az_idx++) {
    for (int el_idx = 0; el_idx < num_elevation_divisions; el_idx++) {
      ...
      const float azimuth_rad = 
          az_idx * azimuth_increments_rad + (M_PI / 2.0 - half_horizontal_fov_rad);
      const float polar_rad =
          el_idx * polar_increments_rad + (M_PI / 2.0 - half_vertical_fov_rad);

      // Spherical to Cartesian
      const float x = radius * sin(polar_rad) * sin(azimuth_rad);
      const float y = radius * sin(polar_rad) * cos(azimuth_rad);
...
DepthImage depthImageFromPointcloud(const Eigen::MatrixX3f& pointcloud,
                                    const Radar& radar) {
  DepthImage depth_image(radar.num_elevation_divisions(),
                         radar.num_azimuth_divisions(), MemoryType::kUnified);
  for (int idx = 0; idx < pointcloud.rows(); idx++) {
    const Vector3f p_C = pointcloud.row(idx);
    Index2D u_C;
    radar.project(p_C, &u_C);
    depth_image(u_C.y(), u_C.x()) = p_C.norm();
  }
  return depth_image;
}

TEST_F(RadarIntegrationTest, RadarAabb) {
  Transform T_L_C = Transform::Identity();
  AxisAlignedBoundingBox aabb = radar.getViewAABB(T_L_C, 0.0f, 1.0f);

  const float left_beam_y = 1.0f * sin(horizontal_fov_rad / 2.0f);
  const float top_beam_z = 1.0f * sin(vertical_fov_rad / 2.0f);

  EXPECT_NEAR(aabb.min().x(), 0.0f, kFloatEpsilon);
  EXPECT_NEAR(aabb.min().y(), -left_beam_y, kFloatEpsilon);
  EXPECT_NEAR(aabb.min().z(), -top_beam_z, kFloatEpsilon);
  EXPECT_NEAR(aabb.max().x(), 1.0f, kFloatEpsilon);
  EXPECT_NEAR(aabb.max().y(), left_beam_y, kFloatEpsilon);
  EXPECT_NEAR(aabb.max().z(), top_beam_z, kFloatEpsilon);

  T_L_C.translate(Vector3f(1.0f, 1.0f, 1.0f));

  aabb = radar.getViewAABB(T_L_C, 0.0f, 1.0f);

  EXPECT_NEAR(aabb.min().x(), 1.0f, kFloatEpsilon);
  EXPECT_NEAR(aabb.min().y(), -left_beam_y + 1.0f, kFloatEpsilon);
  EXPECT_NEAR(aabb.min().z(), -top_beam_z + 1.0f, kFloatEpsilon);
  EXPECT_NEAR(aabb.max().x(), 2.0f, kFloatEpsilon);
  EXPECT_NEAR(aabb.max().y(), left_beam_y + 1.0f, kFloatEpsilon);
  EXPECT_NEAR(aabb.max().z(), top_beam_z + 1.0f, kFloatEpsilon);
}

TEST_F(RadarIntegrationTest, RadarBlocksInView) {
  // Params
  ...
  const Eigen::MatrixX3f pointcloud =
      generateSpherePointcloud(num_azimuth_divisions, num_elevation_divisions,
                               horizontal_fov_rad, vertical_fov_rad, sphere_radius);
  const DepthImage depth_image = depthImageFromPointcloud(pointcloud, radar);
  ...
  std::vector<Index3D> blocks_in_view =
      view_calculator.getBlocksInImageViewRaycast(
          depth_image, T_L_C, radar, voxelSizeToBlockSize(voxel_size),
  ...
  for (const Index3D block_index : blocks_in_view) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      bool did_project = radar.project(p_C, &u_C);
  ...
  if (FLAGS_nvblox_test_file_output) {
    const std::string blocks_in_view_filepath = "./blocks_in_view_radar.csv";
    io::writeToCsv(blocks_in_view_filepath, blocks_in_view_mat);
  }
}

TEST_F(RadarIntegrationTest, SurroundingSphere) {
  // Generate data
  const float sphere_radius = 10.0;
  const Eigen::MatrixX3f pointcloud =
      generateSpherePointcloud(num_azimuth_divisions, num_elevation_divisions,
  ...
  tsdf_integrator.integrateFrame(depth_image, T_L_C, radar, &layer);
  ...
  if (FLAGS_nvblox_test_file_output) {
    // Write out the mesh
    const std::string mesh_filepath = "radar_sphere_mesh.ply";
    io::outputMeshLayerToPly(mesh_layer, mesh_filepath);
    // Output
    const std::string filepath = "sphere_radar_image.csv";
