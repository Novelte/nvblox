...
class Radar {
 public:
  Radar() = delete;
  __host__ __device__ inline Radar(int num_azimuth_divisions,
                                   int num_elevation_divisions,
                                   float horizontal_fov_rad,
                                   float vertical_fov_rad);
  __host__ __device__ inline ~Radar() = default;
  ...
  // Equality
  __host__ inline friend bool operator==(const Radar& lhs, const Radar& rhs);

  // Hash
  struct Hash {
    __host__ inline size_t operator()(const Radar& radar) const;
  };
...
// Equality
__host__ inline bool operator==(const Radar& lhs, const Radar& rhs);

}  // namespace nvblox

#include "nvblox/core/impl/radar_impl.h"
